...
- 8060S 实测 63.65GB 系统内存，但 35B-A3B Uncensored 的 5/5 chat 进程崩溃或自动重载；当前不接入 LiteLLM / `:12342` / `brain-local`，先完成本机资源调参与小模型对照。
...
8060S 当前还不是生产计算节点；必须先解决本机 35B 进程崩溃，并让较小模型通过最小 chat，之后才考虑连通和路由。
```
...
8060S 不直接替换 `qwen-agent`。先完成本机稳定性和小模型对照；本地最小 chat 通过后，再接 `:12342` 并跑 latency、Codex smoke、patch/repo eval。
```
...
- [ ] 8060S 本机稳定性：解决 35B 崩溃，完成 27B/12B 对照；通过后再接 `:12342`

...
| C9 | `labagent-agent` 后端 | 用 `http://82.156.69.153:18020/v1` + `labagent-agent` 跑 C0-C3 | 文本和图片链路已打通；发现 Responses tools 被 router 吃掉，已补透传，待重启复测工具调用 |
...
- `labagent-agent` 的 Codex 文本请求已能返回，说明 C9 文本链路基本通过。
- 公网 `/v1/responses stream=true` 已返回 `text/event-stream`，并包含 `response.completed`。
- Codex 图片请求触发 `vision-local` 后失败，router 返回 `vision-local` / `embed-local` 连接错误。
- 云服务器 `ss -ltnp` 只看到 `:12340` 和 `:18020`，没有 `:12341`；云端 `curl http://127.0.0.1:12341/v1/models` 连接失败。
- 判断：图片失败不是 Codex 协议层问题，也不是 `labagent-agent` 文本路由问题，而是新设备到云端的 `:12341` 反向隧道未运行。恢复新设备 LM Studio 和 `:12341` 隧道后再复测图片输入。

2026-07-03 C9 工具调用观察：

- 恢复新设备 `:12341` 后，图片识别链路已成功，说明 `labagent-agent` 的 image -> `vision-local` 路由已可从远程客户端使用。
- 继续用 Codex 测 `labagent-agent` 文本开发任务时，模型能回答“如何查看目录”“如何找 app.py”，但没有像 `qwen-agent` 直连那样调用 PowerShell 工具。
- 根因：旧 `/v1/responses` 实现把 Codex Responses 请求转成普通 chat completion，丢掉了 Codex 传入的 `tools` 字段。这样 Codex 无法收到模型的工具调用项，只能得到普通文本。
- 修复：`services/agent` 现在对“Responses 请求包含 `tools` 且不含图片”的场景直接透传到上游 `qwen-agent` `/v1/responses`，保留 native Responses/tool 协议；图片请求仍走 `vision-local` side channel。
- 需要重启 5090 上的 `services.agent.server`，再用 `labagent-agent` 复测 C1/C2/C3。通过标准不只是能回答，而是能实际调用 `Get-ChildItem`、读取 `app.py`、创建/修改文件。

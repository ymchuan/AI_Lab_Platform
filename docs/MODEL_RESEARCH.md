...
| 新设备 | RTX 5080 16GB + RTX 4060 Ti 16GB + AMD 集显 | 61.4GB | `embed-local` 已接入 | Embedding 已上线；第二推理 / VL / Rerank 待接入 |
...
2. **RTX 5080 16GB + RTX 4060 Ti 16GB**：最适合承载 RAG 检索侧模型、轻量/中等第二模型、VL/OCR 实验。当前已先接入 `text-embedding-nomic-embed-text-v1.5-embedding` 作为 `embed-local`，减少 5090 被小模型占用。两张卡合计 32GB 是专用显存资源池，不代表单个 dense 模型能天然当作 32GB 单卡运行。
...
5. 新设备已完成 `embed-local` v1 接入，真实 RAG retrieval benchmark 已完成 v0；下一步部署 Qwen3-Embedding / Qwen3-Reranker 对照，并补 answer faithfulness / citation eval，之后评估 VL 与第二代码模型。

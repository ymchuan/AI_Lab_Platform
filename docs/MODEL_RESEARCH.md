...
> 日期：2026-06-10  
> 目标：最大化利用 5090 / 4090D 新设备两台当前可规划机器，同时服务 Agent 开发、RAG、模型工程和简历展示。8060S 当前无法使用，暂不纳入近期资源池。
...
1. **5090 32GB VRAM**：主攻 coding / agent 主模型。优先测试 30B-35B 级模型的 4bit/5bit/6bit 量化，不盲目追 70B+ dense。
2. **4090D 24GB + 4060 Ti 16GB**：最适合承载 RAG 检索侧模型和第二代码模型。Embedding / Reranker 应优先放这里，减少 5090 被小模型占用。两张卡合计 40GB 是资源池，不代表单个 dense 模型能天然当作 40GB 单卡运行。
...
- 4090D 24GB：优先测能单卡稳定运行的中等代码/通用模型，作为 `coder-small-local` 或第二聊天模型。
- 4060 Ti 16GB：优先放 Embedding、Reranker、轻量模型和实验服务。
- 跨 4090D + 4060 Ti 跑单个大模型只作为进阶实验；需要确认 LM Studio / llama.cpp / vLLM / SGLang 对异构双卡的支持和实际吞吐。

| 优先级 | 模型 | 用途 | 部署建议 |
|--------|------|------|---------|
| P0 | Qwen3-Embedding-4B / 8B | RAG 向量化 | 优先放 4090D；小规模也可测 4060 Ti |
...
coder-small-local -> 4090D 上的第二代码模型
```
...
| 维度 | 指标 | 为什么重要 |
|------|------|------------|
| 性能 | first token、tokens/s、总延迟、显存占用 | 证明 5090/4090D 是否被充分利用 |
...
4. 在 5090 上测试 Qwen3-Coder 30B-A3B 和 Qwen3.6-35B-A3B 量化版。
5. 接入新设备，先部署 Qwen3-Embedding / Qwen3-Reranker，再考虑第二代码模型。

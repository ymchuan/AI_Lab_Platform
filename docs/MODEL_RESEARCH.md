...
| P0 (已接入 v1) | qwen/qwen3-vl-30b | 图片问答 / 截图理解 / OCR-ish | 新设备 LM Studio + SSH `:12341` + LiteLLM `vision-local`；待补最小 VL benchmark |
...
5. 新设备已完成 `embed-local` 和 `vision-local` v1 接入，真实 RAG retrieval benchmark 已完成 v0；下一步部署 Qwen3-Embedding / Qwen3-Reranker 对照，补 answer faithfulness / citation eval，并优先验证 `vision-local` 图片问答、截图理解和 OCR-ish 输出质量。

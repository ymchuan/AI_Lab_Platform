...
- 新设备硬件已校准为 RTX 5080 16GB + RTX 4060 Ti 16GB + AMD 集显 + 61.4GB RAM，已通过 `:12341` SSH 反向隧道接入 `embed-local` embedding 路由和 `vision-local` Qwen3-VL-30B 路由。
- 8060S 已恢复为可规划候选节点，但尚未接入 LiteLLM / `:12342` / benchmark；短期只作为 `brain-local` / `doc-local` / `rerank-local` 候选，不替换 5090 主代码模型。
...
│ - 模型别名路由：qwen-local / qwen-agent / embed-local / vision-local │
│ - systemd 后台运行 + 崩溃重启                                  │
│                                                              │
│ SSH Reverse Tunnels                                          │
│ - :12340 -> 5090:1234    手动开启                              │
│ - :12341 -> 新设备:1234  Nomic embedding + Qwen3-VL             │
│ - :12342 -> 8060S:1234   候选 brain / doc / rerank（未上线）       │
│                                                              │
...
│ 本地候选模型池    │                      │ LM Studio         │
│ ✅ 已部署         │                      │ ✅ Embedding / Vision 已接入 │
└──────────────────┘                      └──────────────────┘

8060S 已恢复为候选节点，但当前还不是生产计算节点；先完成连通、路由和 benchmark 后再决定角色。
```
...
3. 多后端：当前正式后端是 5090 和 5080 新设备；8060S 已恢复为下一候选后端，需先经过 `:12342` 路由和 benchmark。
...
8060S (Ryzen AI Max+ 395)    -> 候选 brain / 文档处理 / Rerank / 轻量服务
云服务器 (2GB)               -> LiteLLM / HTTPS / 鉴权 / SSH 隧道中转

8060S 不直接替换 `qwen-agent`。先接 `:12342` 并跑 latency、Codex smoke、patch/repo eval，再决定是否承接 `brain-local`、`doc-local`、`rerank-local` 或 `coder-small-local`。
```
...
5. LiteLLM 路由到 qwen-local / qwen-agent / embed-local / vision-local 后端
...
```text
1. 客户端请求 POST /v1/chat/completions，model=labagent-agent
2. router 检查是否存在 image_url 内容块
3. 若有图像，先调用 vision-local 提取视觉摘要
4. 若命中 LabAgent / 项目知识关键词，调用 RAG Service
5. router 把视觉摘要、RAG 证据和原始文本交给 qwen-agent
6. qwen-agent 输出最终面向用户的工程回答
```
...
```text
1. 客户端请求 POST /v1/chat/completions，model=vision-local
2. LiteLLM 验证 API Key
3. 请求通过 SSH :12341 转发到新设备 127.0.0.1:1234
4. 新设备 LM Studio 调用 `qwen/qwen3-vl-30b`
5. 返回图片描述、截图理解或 OCR-ish 文本结果
```

8060S 候选请求的数据流：

```text
1. 8060S 本机 LM Studio 启动目标模型
2. 8060S 本机开启 SSH :12342 -> 127.0.0.1:1234 反向隧道
3. 云端 LiteLLM 增加 brain-local / doc-local / rerank-local 等 alias
4. 先通过 /v1/models、/v1/chat/completions 和 benchmark 验证
5. 通过前只作为实验路由，不进入团队默认 qwen-agent
```
...
当前 `services/agent` 还不是完整 Agent Runtime。它只做 route + side channel compose，不做 shell / file / memory / planner 执行。
...
- [x] 新设备 `vision-local` v1 接入：LM Studio + Qwen3-VL-30B + SSH `:12341` + LiteLLM 路由
- [x] 验证 `vision-local` 图片问答、截图理解和 OCR-ish 输出质量，并补最小 VL benchmark
- [ ] 调研并确定 5080 新设备的 Reranker / 第二代码模型组合
- [x] 建立本地 benchmark v2：网关健康、吞吐、首 token 延迟、Agent 任务、RAG oracle、repo map、patch、Cline 多轮对话
- [x] LiteLLM 多节点基础路由：`qwen-local` / `qwen-agent` / `embed-local` / `vision-local`
- [x] RAG v0：Markdown chunking + `embed-local` + 本地 JSON index + retrieval benchmark + cited answer
- [ ] 云服务器轻量 HTTPS 入口（Nginx 或 Caddy）
- [ ] 8060S 候选节点接入：`:12342` 隧道、LiteLLM alias、latency 和 Codex smoke

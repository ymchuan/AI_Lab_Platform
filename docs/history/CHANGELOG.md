...
- 新增 `-VisionOnly`，允许候选视觉模型只跑一个合成图片请求，避免为图片验收重复执行整套慢速文本 smoke。
...
### Added
- 为七个文档分类新增短 README，分别提供读者角色、最小阅读顺序、完成标准和跨分类入口。

### Changed
- 将根 docs 门户收敛为路线选择页，详细文件清单下沉到分类目录，避免新人第一次打开就面对完整文档列表。
...
### Changed
- 实际去重四份正文：将 Progress Summary 收敛为阶段报告，将 Agent 项目路线收敛为里程碑与验收门槛，将 AI Engineer 技能路线收敛为能力缺口与学习顺序，将团队客户端文档收敛为支持矩阵。
- 为每份总览明确了链接边界：详细 RAG/Agent 设计进入项目深挖文档，Codex C1-C9 细节进入 Codex 兼容性文档，历史过程进入项目日志。
- 扩充文档地图和同步契约，要求新增文档前先判断能否并入既有专题，避免再次形成平行总览。

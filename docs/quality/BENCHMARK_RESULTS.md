...
### Gemma 31B VisionOnly

Run `20260716_204037` 只执行合成图片 case。脚本正确跳过全部文本任务，但 `t06_vision` 在 45.944s 后返回模型进程崩溃，退出码仍为 `18446744072635812000`，无 content/reasoning/token。该轮 `lms ps` 显示 context 16016、parallel 4，明显高于此前成功文本轮的资源配置。

结论：当前配置下 Gemma 31B 不能替换 `vision-local`，不得接入 `vision-candidate`。只允许最后一次控制变量复测：卸载后以 context 4096、parallel 1、关闭 speculative decoding 重新加载，再运行同一个 `-VisionOnly`。若仍崩溃，立即停止 Gemma Vision 路线；8060S 改作 document/offline text side，视觉继续保留现有 VL 或迁移到更小的专用 VL 模型。

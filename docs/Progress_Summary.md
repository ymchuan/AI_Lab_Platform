...
```text
本地 GPU 主机（5090 / 5080 新设备 / 8060S 候选）
    ↓ SSH Reverse Tunnel
云服务器（公网轻量 API 网关）
...
| 8060S | AMD Ryzen AI MAX+ 395 / Radeon 8060S / NPU | 31.6GB | 🧪 已恢复为候选节点，未接入路由，待 benchmark |
...
✅ 2026-07-02 复测 Codex C9：`labagent-agent` 文本链路已通过；首轮图片失败定位为新设备 `:12341` 反向隧道未运行
✅ 2026-07-03 恢复新设备 `:12341` 后，`labagent-agent` 图片识别链路已由远程客户端验证成功
✅ 2026-07-03 定位 C9 第二个问题：Codex `tools` 被旧 router 转 chat 时丢失，已改为无图片 Responses tools 请求透传到上游 `qwen-agent`
✅ 2026-07-03 8060S 恢复为候选节点，已明确为先接 `:12342` + benchmark，不直接替换 5090 `qwen-agent`
✅ 2026-07-09 新增 `scripts/start_5090_services.ps1`，把 5090 上的 `qwen-tunnel`、RAG、Agent Router 和公网隧道启动命令收敛到一个脚本；复测公网 `labagent-agent` direct chat 返回 `pong`
✅ 2026-07-10 新增 `scripts/check_labagent_status.ps1` 每日巡检脚本；首次巡检核心链路 14 OK、0 FAIL，公网 `labagent-agent`、`qwen-agent`、`embed-local`、`vision-local` 均通过 smoke test
```

当前边界：

```text
`stream=true` 目前分两类：router 编排请求仍是 SSE 兼容降级；Codex tools 请求已改为透传上游 native Responses 流
Codex C9 文本链路已确认 Responses streaming 修复在线上生效，但 tools 透传需要重启服务后复测
图片输入依赖新设备 `:12341`；该隧道断开时 `vision-local` 和 `embed-local` 会一起失败。恢复后图片链路已通，但代码截图质量仍需 benchmark
Agent Router 必须加载 `.env.local` 或显式传入 `--base-url http://82.156.69.153:8000/v1`；否则会回落到本机不存在的 `127.0.0.1:8000/v1`，导致 David/Cline 侧表现为 connection refused / 502
router 自身还没有 tool execution；Codex 的 shell/file 工具由 Codex CLI 在透传 Responses 协议后执行
...
云服务器 (Ubuntu 24.04, 2GB)
    ...
    └── SSH 隧道中转
         ...
         └── :12342 -> 8060S Ryzen AI Max+ 395 / Radeon 8060S（候选，未上线）
```

8060S 已恢复为候选节点，但暂不纳入团队默认路由；先跑本机、隧道、LiteLLM alias 和 benchmark。
...
| 8060S | 候选 brain / 文档处理 / rerank / 轻量服务 | 先接 `:12342`，验证后再决定是否进入 router |
| 云服务器 | LiteLLM + HTTPS + 鉴权 + 隧道中转 | 轻量公网入口 |

OCR / Whisper / 文档解析能力仍保留在项目路线中；8060S 恢复后可以重新作为候选部署位置，但必须和新设备方案做同一套吞吐、稳定性和维护成本对照。
...
| 8060S 已恢复但未评测 | 还没有 `:12342`、LiteLLM alias 和 benchmark 证据 | 作为候选节点接入，先测 `brain-local` / `doc-local` / `rerank-local`，不直接替换 5090 主代码模型 |
| 5090 手动启动命令容易出错 | RAG、Agent Router 和 SSH 隧道都依赖多个 PowerShell 窗口，且 Agent Router 未加载 env 时会连错本机 `:8000` | 已新增 `scripts/start_5090_services.ps1`，用 `-Action qwen-tunnel/rag/rag-tunnel/agent/agent-tunnel/status` 分窗口启动和排查 |
| 日常状态检查靠手工 curl 容易漏项 | 端口监听、真实 API 调用、RAG、Vision、Agent Router 分属不同入口 | 已新增 `scripts/check_labagent_status.ps1`，每天运行一次即可看到 OK/WARN/FAIL 和下一步建议 |
...
| 团队 Codex CLI 兼容性需要系统化 | 基础 smoke 已通过，2026-06-30 又通过 C1-C6 小型开发 workflow；2026-07-02 复测确认 `labagent-agent` 文本链路通过；2026-07-03 恢复新设备 `:12341` 后图片链路已通，并修复 C9 tools 请求被 router 吃掉的问题 | 下一步重启 8020 后复测 C9 C1-C3，再测 C7 长上下文、C8 后端异常错误体验，并扩展图片质量 benchmark |
...
1. 优先完成 Codex CLI 团队接入兼容性矩阵：C1-C6 已通过，C9 文本和图片链路已通过；下一步重启 `labagent-agent` 后验证 C9 tools 透传是否真的触发 PowerShell / 文件工具，再测 C7 长上下文和 C8 错误处理。
2. 接入 8060S 候选节点：本机 LM Studio、`:12342` 隧道、LiteLLM alias、latency、Codex smoke、patch/repo eval；通过前不迁移 `qwen-agent`。
3. 将 RAG v1.x 升级到 Qdrant/Chroma、reranker、answer eval。
4. 固化新设备 `vision-local` 图片问答、截图理解和 OCR-ish 最小 benchmark，并接入 Reranker、第二代码模型。
5. 扩展团队客户端兼容性：Codex CLI 已通过基础 chat/read/write 和单文件 Python patch smoke，下一步测多文件编辑、长上下文和错误处理；Claude Code CLI 继续单独处理 tool use schema。
6. 固定 5090 load Qwen3-Coder 30B，继续补工具调用、repo map、patch apply 和 Cline 多轮真实工作流评测。
7. 构建 Agent Runtime：工具调用、任务规划、文件/代码工具、权限控制、持久化 trace。
8. 开发 MCP Server / Skills / Eval Harness，并做 LoRA/QLoRA 与量化实验。
9. 把 Claude Code 兼容性单独拆成评测项，固定最小复现样本，优先确认 tool use 参数错误发生在哪一层，再决定是适配、约束还是降级。
10. 把 `labagent-agent` 从编排层继续往前推：先验证 Responses streaming / tools 透传、错误恢复和图像回放，再进入真正的 planner/tool registry。

## 八、简历表达（当前版本）

> 设计并搭建一套基于本地 GPU 与云服务器中转的私有 AI API 网关平台。针对校园网 NAT 环境下本地 RTX 5090 推理节点无法直接公网访问的问题，使用 SSH Reverse Tunnel 将内网模型服务映射至腾讯云 Ubuntu 24.04 服务器，并通过 LiteLLM 封装为 OpenAI-compatible API，支持 Codex CLI、Cline、OpenWebUI、Python/JS 客户端通过统一 Base URL 调用本地模型。项目已建立 8 层 benchmark，对 Qwen3-Coder-30B、Qwen3.6-27B、Qwen3.6-35B-A3B、Qwen3-30B-A3B、Gemma、GLM、embedding 和 vision 模型进行对比，当前将 Qwen3-Coder-30B 定为 5090 `qwen-agent` 默认模型，并基于新设备 `embed-local` 实现 RAG Service v1：Markdown chunking、本地向量索引、检索评测、带引用回答、OpenAI-compatible HTTP API 和公网远程 health 验证；`vision-local` 与 `labagent-agent` 图片链路已通过远程 smoke，用于图片问答和截图理解。8060S 已恢复为候选节点，下一步将按 `:12342` 路由、latency、Codex smoke 和 patch/repo eval 验证后再决定是否承担 brain、文档处理、rerank 或第二代码模型。下一阶段将扩展 RAG v1.x、Agent Runtime、MCP Server、工具调用评测、量化与微调实验，目标是形成面向 Agent 开发岗的端到端 AI Infra 项目。

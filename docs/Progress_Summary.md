...
✅ vision-local 路由已接入；图片识别质量和消息格式兼容性待最小 benchmark 验证
```
...
| 新设备已接入 embedding / vision | 多节点路由 v1 已完成，但 Reranker/第二代码模型未接入，Vision 仍待质量评测 | RAG v0 已使用 `embed-local`；下一步验证 `vision-local` 并扩展 Reranker |
...
**当前 5090 默认 `qwen-agent` 定为 Qwen3-Coder-30B；`embed-local` 已支撑 RAG Service v1；`vision-local` 已接入但待图片识别评测；后续 benchmark 重点转向真实工具调用、RAG answer faithfulness、rerank、VL 和多节点路由。**

## 八、下一步计划

1. 启动 RAG Service v1，从 David 验证 `/health`、`/v1/rag/search`、`/v1/rag/ask`。
2. 将 RAG v1.x 升级到 Qdrant/Chroma、reranker、answer eval。
3. 固定 5090 load Qwen3-Coder 30B，继续补工具调用、repo map、patch apply 和 Cline 多轮真实工作流评测。
4. 验证新设备 `vision-local` 图片问答、截图理解和 OCR-ish 输出质量，并接入 Reranker、第二代码模型。
5. 构建 Agent Runtime：工具调用、任务规划、文件/代码工具、权限控制、持久化 trace。
6. 开发 MCP Server / Skills / Eval Harness，并做 LoRA/QLoRA 与量化实验。
7. 把 Claude Code 兼容性单独拆成评测项，固定最小复现样本，优先确认 tool use 参数错误发生在哪一层，再决定是适配、约束还是降级。

## 八、简历表达（当前版本）

> 设计并搭建一套基于本地 GPU 与云服务器中转的私有 AI API 网关平台。针对校园网 NAT 环境下本地 RTX 5090 推理节点无法直接公网访问的问题，使用 SSH Reverse Tunnel 将内网模型服务映射至腾讯云 Ubuntu 24.04 服务器，并通过 LiteLLM 封装为 OpenAI-compatible API，支持 Cline、OpenWebUI、Python/JS 客户端通过统一 Base URL 调用本地模型。项目已建立 8 层 benchmark，对 Qwen3-Coder-30B、Qwen3.6-27B、Qwen3.6-35B-A3B、Qwen3-30B-A3B、Gemma、GLM、embedding 和 vision 模型进行对比，当前将 Qwen3-Coder-30B 定为 5090 `qwen-agent` 默认模型，并基于新设备 `embed-local` 实现了 RAG v0：Markdown chunking、本地向量索引、检索评测和带引用回答；`vision-local` 已接入用于图片问答和截图理解，待补最小 VL benchmark。下一阶段将扩展 RAG Service、Agent Runtime、MCP Server、工具调用评测、量化与微调实验，目标是形成面向 Agent 开发岗的端到端 AI Infra 项目。

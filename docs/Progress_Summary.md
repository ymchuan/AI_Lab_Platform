...
| 团队 Codex CLI 兼容性需要系统化 | 基础 smoke 已通过，2026-06-30 又通过 C1-C6 小型开发 workflow；2026-07-02 复测确认 `labagent-agent` 文本链路通过；2026-07-03 恢复新设备 `:12341` 后图片链路已通 | 下一步测 C7 长上下文、C8 后端异常错误体验，并扩展图片质量 benchmark |
...
1. 优先完成 Codex CLI 团队接入兼容性矩阵：C1-C6 已通过，C9 文本和图片链路已通过；下一步验证 C7 长上下文和 C8 错误处理。
...
10. 把 `labagent-agent` 从编排层继续往前推：先验证 Responses streaming 兼容、错误恢复和图像回放，再进入真正的 planner/tool registry。

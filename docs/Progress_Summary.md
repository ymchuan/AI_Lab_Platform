...
| 新设备 | RTX 5080 16GB + RTX 4060 Ti 16GB + AMD 集显 | 61.4GB | ⏳ 未接入 |
...
云服务器 (Ubuntu 24.04, 2GB)
    ...
    └── SSH 隧道中转
         ...
         ├── :12341 -> 新设备 RTX 5080 16GB + RTX 4060 Ti 16GB（待接入）
```
...
**当前 5090 默认 `qwen-agent` 定为 Qwen3-Coder-30B；后续 benchmark 重点转向真实工具调用、RAG retrieval 和多节点路由。**

## 八、下一步计划

1. 固定 5090 load Qwen3-Coder 30B，继续补工具调用、repo map、patch apply 和 Cline 多轮真实工作流评测。
2. 接入新设备（5080 + 4060 Ti）：先跑 SSH 隧道 :12341、Embedding + Reranker，再评估 VL/第二代码模型。

...
| 设备 | GPU / 加速器 | 内存 | 当前状态 |
|------|-------------|------|---------|
| 5090 | RTX 5090 32GB + AMD Radeon 610M | 93.7GB | ✅ 已接入，运行 `qwen/qwen3.6-27b` GGUF Q6_K |
| 新设备 | RTX 4090D 24GB + RTX 4060 Ti 16GB + AMD 集显 | 61.6GB | ⏳ 未接入 |
...
LiteLLM (systemd 后台服务，开机自启，崩溃重启)
    └── qwen-local -> 5090 当前 qwen/qwen3.6-27b 后端
```
...
| 新设备未接入 | 待配置 | 先完成模型选型和 benchmark，再接入隧道与路由 |
| 新设备显存不是连续 40GB | 4090D 24GB 和 4060 Ti 16GB 是两张独立 GPU | 按总资源池规划，优先分配不同模型；跨卡单模型作为进阶实验 |
| 8060S 暂不可用 | 设备当前无法使用 | 冻结近期接入，相关任务迁移到新设备或后移 |
| 当前项目代码深度不足 | 主要是部署和文档 | 下一阶段补 RAG、Agent、MCP、Eval、微调/量化实验 |
| 当前 5090 模型正在选型 | 已测试 Qwen3.6-27B、Qwen3-Coder-30B、Qwen3.6-35B-A3B、GLM-4.7-Flash、Gemma 4 31B | 以 Qwen3-Coder-30B 为当前首选候选，继续补真实 Agent harness |
...
| 模型 | 测试日期 | 结果 |
|------|---------|------|
| qwen/qwen3.6-27b GGUF Q6_K | 2026-06-10 / 06-15 | 基线模型；reasoning 输出过多，Agent 任务 0/3 通过 |
| qwen/qwen3-coder-30b | 2026-06-15 / 06-16 | 当前最强 coding / patch / agent-readiness 候选；patch 2/2，agent_tasks strict 2/4、soft 4/4，cline_dialogue strict 0/2、soft 2/2 |
| qwen/qwen3.6-35b-a3b | 2026-06-15 | 速度快，但 reasoning 挤占输出；patch/agent/Cline 失败 |
...
| text-embedding-nomic-embed-text-v1.5 | 2026-06-16 | `/v1/embeddings` 可用，768 维；toy retrieval 2/3，后续补真实 RAG retrieval benchmark |
...
**谁在 benchmark 里最终 `content` 更稳定、工具选择更准、吞吐更可接受，就让谁当 `qwen-local`。**

## 八、下一步计划

1. 以 Qwen3-Coder 30B 为当前首选候选，继续补工具调用、repo map、patch apply 和 Cline 多轮真实工作流评测。
2. 确定基座模型后，接入新设备（Embedding + Reranker + SSH 隧道 :12341）。
3. 构建 RAG 知识库系统：文档 ingestion、embedding、向量库、rerank、引用溯源。
4. 构建 Agent Runtime：工具调用、任务规划、文件/代码工具、权限控制、持久化 trace。
5. 开发 MCP Server / Skills / Eval Harness，并做 LoRA/QLoRA 与量化实验。

## 八、简历表达（当前版本）

> 设计并搭建一套基于本地 GPU 与云服务器中转的私有 AI API 网关平台。针对校园网 NAT 环境下本地 RTX 5090 推理节点无法直接公网访问的问题，使用 SSH Reverse Tunnel 将内网模型服务映射至腾讯云 Ubuntu 24.04 服务器，并通过 LiteLLM 封装为 OpenAI-compatible API，支持 Cline、OpenWebUI、Python/JS 客户端通过统一 Base URL 调用本地 `qwen/qwen3.6-27b` GGUF Q6_K 模型。项目下一阶段将扩展多节点模型路由、RAG Service、Agent Runtime、MCP Server、模型评测、量化与微调实验，目标是形成面向 Agent 开发岗的端到端 AI Infra 项目。

...
| 团队 Codex CLI 兼容性需要系统化 | 基础 smoke 已通过，但多文件编辑、测试执行和失败恢复还没矩阵验证 | 当前 P0 改为 `Codex CLI + qwen-agent` 兼容性矩阵，新增 `docs/CODEX_CLI_COMPATIBILITY.md` 和 `benchmarks/fixtures/codex_cli_smoke` |
...
1. 优先完成 Codex CLI 团队接入兼容性矩阵：用 `benchmarks/fixtures/codex_cli_smoke` 验证多文件编辑、运行测试、失败修复、长上下文和错误处理。
...
9. 把 `labagent-agent` 从编排层继续往前推：先补 streaming、错误恢复和图像回放，再进入真正的 planner/tool registry。
